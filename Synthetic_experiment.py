...
from utils.modelutils import QUANN, MeanSet, MLP
...
NUM_EPOCHS = 10
LATENT_DIM = 16
OUTPUT_DIR = './results/Synthetic_experiment/'
...
def make_datasets(agg_func_name):
    ...
    for n, s in size.items():
      data[n] = make_vector_aggregates(
        ...
        dim = INPUT_DIM,
        num = s,
        min_k = 2 if agg_func_name in ['skewness', 'variance', 'kurtosis'] else 1,
        max_k = MAX_K
      )  
  ...
  for replicate in range(NUM_REPLICATES):
    ...
    for agg_func_name in AGG_FUNCS:
      ...
      for lr in LEARNING_RATEs:
          ...
          models['Ablation_2'] = MeanSet(
              latent_dim=LATENT_DIM,
              input_dim=INPUT_DIM,
              output_dim=INPUT_DIM,
              encoder=MLP(INPUT_DIM, LATENT_DIM, hidden_dims=[128, 32, 32]),
              predictor=MLP(LATENT_DIM, INPUT_DIM, hidden_dims=[32, 32, 128])                            
          )               
